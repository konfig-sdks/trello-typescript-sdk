...
import { readableStreamToString, TrelloError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new TrelloError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new TrelloError(e, e.response?.data)
                }
                if (innerError instanceof TrelloError) {
                    // Got "TrelloError" from the above try block
                    throw innerError;
                }
